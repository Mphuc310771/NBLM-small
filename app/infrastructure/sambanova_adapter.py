...
class SambaNovaAdapter(ILLMService):
    def __init__(self, api_key: str = None, model: str = None):
        """
        SambaNova LLM service adapter.
    ...
    def generate_answer(self, context: str, query: str, system_prompt: str = None, provider: str = "sambanova") -> Generator[str, None, None]:
        if not self.api_key:
            raise ValueError("SAMBANOVA_API_KEY must be set in .env before using SambaNova.")
        ...
        try:
            logger.info(f"Sending request to SambaNova API (model: {self.model})...")
            ...
            logger.error(f"SambaNova API streaming error: {e}", exc_info=True)
            raise e
